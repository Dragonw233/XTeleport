...
public static class ZoneInfoHandler
{
    ...
    public static void Init()
    {
        ...
        var contentFinderSheet = Svc.Data.GetExcelSheet<ContentFinderCondition>()!;
        ...
        foreach (var zone in territorySheet)
        {
            ...
            {
                var dutyName = StaticUtils.ToStr(contentRow.Name).Trim();
                if (dutyName.Length > 0)
                    dutyName = dutyName.First().ToString().ToUpper() + dutyName[1..];

                ZoneInfoDict.Add(
                    (ushort) zone.ContentFinderCondition.RowId,
                    new ZoneInfo(dutyName, StaticUtils.ToStr(zone.PlaceName.Value!.Name), zone.RowId, zone.Map.Value!.Id.ToString().Split('/')[0], (ushort) zone.ContentFinderCondition.RowId, contentRow.Content.RowId));
            }
